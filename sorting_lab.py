...
This is a little different from the sorts done in class.  You will need to sort nba_leader_list[i] by using the score located at nba_leader_list[i][1].
This is designed to test your understanding of the sort by applying it to slightly more messy list.  Solving malformed problems leads to deeper understanding.  Good luck (25pts)
...
#remember to move PLAYER and SCORE at the SAME TIME

for pos in range(len(nba_leader_list)):
    min_pos = pos
    for scan_pos in range(min_pos, len(nba_leader_list)):
        if nba_leader_list[scan_pos][1] < nba_leader_list[min_pos][1]:
            min_pos = scan_pos
